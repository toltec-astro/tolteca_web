...
class ToltecSignalFitsViewer(ComponentTemplate):
    ...
    def setup_layout(self, app):
        ...
        arrayTitle = mainControlsTitlesRow.child(dbc.Col, width=3).child(
            html.H5, "Array Choice", className="mb-2"
        )
        wtcutTitle = mainControlsTitlesRow.child(dbc.Col, width=3).child(
            html.H5, "Weight Cut", className="mb-2"
        )
        tzTitle = mainControlsTitlesRow.child(dbc.Col, width=2).child(
            html.H5, "Trim Map Zeros", className="mb-2"
        )
        ...
        arrayChoice = arrayCol.child(
            dcc.RadioItems,
            options=[
                {"label": "a1100", "value": "a1100", "disabled": True},
        ...
        trimEdgeSel = trimEdgeCol.child(daq.ToggleSwitch, default=True)
        mainControls = {
            "array": arrayChoice,
            "weightCut": weightCut,
        ...
        # Map Histogram
        imageOptions = [
            {"label": a, "value": a} for a in ["signal", "weight", "sig2noise"]
        ]
        ...
        pixelHistogram = {"controls": histControl, "plot": histPlot}

        super().setup_layout(app)
        self._registerCallbacks(
            app, fitsPath_select, pixelHistogram, images, mainControls
        )
        return

    def _registerCallbacks(
        self, app, fitsPath_select, pixelHistogram, images, mainControls
    ):
        ...
        def primaryDropdown(path, histImage, weightCut, arrayOpt, array):
            ...
            if len(availableArrays) == 0:
                raise ValueError("No FITS files available for use at this path.")
            if array is None:
        ...
        def threePanelFig(path, weightCut, array, trimEdges, *sp_inputs_list):
            if (path is None) | (path == ""):
                raise PreventUpdate
            outputs = []
            for sp_inputs, name, signal in zip(
                sp_inputs_list,
                ["signal", "weight", "s2n"],
                ["signal_I", "weight_I", "sig2noise_I"],
            ):
                image_data, _ = getImage(
                    signal, path, weightCut, array, trimEdge=trimEdges
                )
