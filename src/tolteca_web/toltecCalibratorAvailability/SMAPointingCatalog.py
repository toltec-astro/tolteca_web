...
class SMAPointingCatalog(object):
    ...
    def __init__(self, filepath):
        ...
        # Generate a set of astropy tables.
        #   self.sources is all of the sources
        #   self.brightest is all sources with 1mm flux > 1Jy
        self.makeAstropyTable()
    ...
    def makeAstropyTable(self, flux_cut=2.0):
        ...
        plusminus = u"\u00B1"
        for ps in pointingSources:
            ps['coords'] = SkyCoord(ps['ra'], ps['dec'],
                           unit=(u.hourangle, u.deg),
                           frame='icrs')

        # convert to pandas dataframe and then to astropy table
        df = pd.DataFrame(pointingSources)
        t = Table.from_pandas(df)

        # get rid of everything but the 1mm measurements
        dates = []
        fluxes = []
        for s in t:
            freq_list = ast.literal_eval(s['freq'])
            date_list = ast.literal_eval(s['date'])
            flux_list = ast.literal_eval(s['flux'])
            i = [i for i, f in enumerate(freq_list) if f == '1mm']
            if(len(i) == 1):
                dates.append(date_list[i[0]])
                fluxes.append(flux_list[i[0]])
            elif(len(i) == 0):
                dates.append(None)
                fluxes.append(None)
            else:
                dates.append(date_list[0])
                fluxes.append(flux_list[0])
        t['1mm date'] = [Time(convert_to_iso(date), format='iso', scale='utc') for date in dates]
        t['1mm flux value'] = [float(psf.split(plusminus)[0]) for psf in fluxes]
        t['1mm flux error'] = [float(psf.split(plusminus)[1]) for psf in fluxes]

        # Let's clean up the formatting of the table a bit.
    ...
    # Find the bright sources currently above 50 deg. elevation.
...
if __name__ == "__main__":
    sma = SMAPointingCatalog("./smaSources.csv")
