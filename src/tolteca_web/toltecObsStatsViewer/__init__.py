...
from ToltecObsStats import ToltecObsStats
...
class ToltecObsStatsViewer(ComponentTemplate):
    ...
    def setup_layout(self, app):
        ...
        a11Col = imageRow.child(dbc.Col, width=4)
        a11ff = getFancyFigContainer(a11Col)
        a14Col = imageRow.child(dbc.Col, width=4)
        a14ff = getFancyFigContainer(a14Col)
        a20Col = imageRow.child(dbc.Col, width=4)
        a20ff = getFancyFigContainer(a20Col)
        threeStat = {'a1100': a11ff,
                     'a1400': a14ff,
                     'a2000': a20ff,}
        ...
        # The array view
        arrayViewRow = bigBox.child(dbc.Row)
        a11ViewCol = arrayViewRow.child(dbc.Col, width=4)
        a11View = getFancyFigContainer(a11ViewCol)
        a14ViewCol = arrayViewRow.child(dbc.Col, width=4)
        a14View = getFancyFigContainer(a14ViewCol)
        a20ViewCol = arrayViewRow.child(dbc.Col, width=4)
        a20View = getFancyFigContainer(a20ViewCol)
        arrayView = {'a1100': a11View,
                     'a1400': a14View,
                     'a2000': a20View,}
        
        self._registerCallbacks(app,
                                statsList,
                                threeStat,
                                mainControls,
                                arrayView,)
    ...
    def _registerCallbacks(self, app,
                           statsList,
                           threeStat,
                           mainControls,
                           arrayView,):
...
def getFancyFig(to, name, array, ranges):
    ...
    if(bs):
        imfig = px.imshow(image, zmin=ranges[0], zmax=ranges[1],
...
def getAVAnimation(to, name, array, ranges, chunk=None):
    ...
    imfig = px.scatter(df, x='x_t', y='y_t', color='value', range_color=ranges,
